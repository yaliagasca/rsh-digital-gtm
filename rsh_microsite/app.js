...
function riskMatrix(){
  ...
  return `<div class="risk-matrix-wrap"><div class="risk-matrix-title">Risk management matrix</div><div class="risk-table" role="table" aria-label="Risk management matrix"><div class="risk-table-header" role="row"><div role="columnheader">Risk</div><div role="columnheader">Potential impact</div><div role="columnheader">Impact level</div><div role="columnheader">How we address it</div></div>${rows.map((r,i)=>`<div class="risk-table-row risk-row-${i+1}" role="row"><div class="risk-cell risk-name" role="cell">${esc(r[0])}</div><div class="risk-cell" role="cell">${esc(r[1])}</div><div class="risk-cell risk-level" role="cell"><span>${esc(r[2])}</span></div><div class="risk-cell" role="cell">${esc(r[3])}</div></div>`).join('')}</div></div>`;
}

function buildSection(source,prefix){const groups=parseGroups(source,prefix);const tabs=groups.map((g,i)=>{const m=g.title.match(/^([AB]\.\d+)\s+(.+)$/);return `<button class="section-tab ${i===0?'is-active':''}" type="button" data-tab="${prefix}-tab-${i}"><span>${esc(m?m[1]:'')}</span>${esc(m?m[2]:g.title)}</button>`;}).join('');const panels=groups.map((g,i)=>{const m=g.title.match(/^([AB]\.\d+)\s+(.+)$/);const number=m?m[1]:'';const title=m?m[2]:g.title;const{intro,sections}=splitSubsections(g.items,prefix);const extra=prefix==='B'&&number==='B.10'?riskMatrix():'';return `<section class="section-tab-panel ${i===0?'is-active':''}" id="${prefix}-tab-${i}"><div class="proposal-block-heading"><span>${esc(number)}</span><h3>${esc(title)}</h3></div><div class="proposal-content">${intro.length?`<div class="intro-copy interactive-highlight">${intro.map(renderText).join('')}</div>`:''}${sections.length?`<div class="accordion-list">${sections.map(accordion).join('')}</div>`:''}${extra}</div></section>`;}).join('');return `<div class="section-tabs">${tabs}</div>${panels}`;}

function bind(container){const tabs=container.querySelectorAll('.section-tab');const panels=container.querySelectorAll('.section-tab-panel');tabs.forEach(tab=>tab.addEventListener('click',()=>{tabs.forEach(t=>t.classList.remove('is-active'));panels.forEach(p=>p.classList.remove('is-active'));tab.classList.add('is-active');document.getElementById(tab.dataset.tab).classList.add('is-active');}));container.querySelectorAll('.accordion-button').forEach(btn=>btn.addEventListener('click',()=>{const card=btn.closest('.content-accordion');const open=card.classList.toggle('is-open');btn.setAttribute('aria-expanded',String(open));btn.querySelector('.accordion-icon').textContent=open?'−':'+';}));}
