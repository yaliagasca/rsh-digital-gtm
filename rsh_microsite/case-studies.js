(() => {
  ...
  const allTags = [...new Set(cases.flatMap(item => [...item.sectorTags, ...item.capabilityTags]))].sort((a,b)=>a.localeCompare(b));
  const selected = new Set();
  const iconFor = item => { const t=[...item.sectorTags,...item.capabilityTags].join(' ').toLowerCase(); if(t.includes('health'))return'health'; if(t.includes('transport')||t.includes('automotive')||t.includes('fleet'))return'mobility'; if(t.includes('food')||t.includes('cpg'))return'growth'; if(t.includes('financial'))return'finance'; return'strategy'; };
  const iconSvg = name => ({
  ...
  const card=item=>`<article class="case-card" data-id="${item.id}"><div class="case-card-topline"><span class="case-card-icon case-icon-${iconFor(item)}">${iconSvg(iconFor(item))}</span><span class="case-card-sector">${item.sectorTags.join(' · ')}</span></div><p class="case-card-client">${item.clientDisplayName}</p><h3>${item.engagementTitle}</h3><p class="case-card-summary">${item.summary}</p><div class="case-card-tags">${tagsFor(item).slice(0,5).map(tag=>`<span>${tag}</span>`).join('')}</div><button class="case-card-open" type="button" data-open-case="${item.id}" aria-label="Open ${item.engagementTitle}">Explore case <span aria-hidden="true">↗</span></button></article>`;

  function renderActiveFilters(){
    activeFilters.innerHTML=[...selected].map(tag=>`<button type="button" class="case-active-chip" data-remove-tag="${tag}" aria-label="Remove ${tag} filter"><span>${tag}</span>×</button>`).join('');
    activeFilters.hidden=selected.size===0;
    activeCount.textContent=selected.size;
    activeCount.hidden=selected.size===0;
    activeFilters.querySelectorAll('[data-remove-tag]').forEach(btn=>btn.addEventListener('click',()=>{selected.delete(btn.dataset.removeTag);render();}));
  }

  function render(){
    const filtered=selected.size?cases.filter(item=>tagsFor(item).some(tag=>selected.has(tag))):cases;
    grid.innerHTML=filtered.map(card).join('');
    resultSummary.innerHTML=`<strong>${filtered.length}</strong> case ${filtered.length===1?'study':'studies'}`;
    empty.hidden=filtered.length!==0; grid.hidden=filtered.length===0;
    root.querySelector('.case-clear').disabled=selected.size===0;
    root.querySelectorAll('.case-filter').forEach(button=>{const active=selected.has(button.dataset.tag);button.classList.toggle('is-active',active);button.setAttribute('aria-pressed',String(active));});
    renderActiveFilters();
